# safe-command 仕様書

## 概要

**safe-command** は、AIエージェント（Claude Code等）が実行できるコマンドを安全に制限するプロキシツールです。

### 背景

Claude Codeでは、`Bash(aws * get-:*)`のような柔軟なパターンマッチングによるコマンド制限ができません。そこで、safe-commandをプロキシとして使用することで、設定ファイルベースで特定のコマンドパターンのみを許可する仕組みを提供します。

### 目的

- AIエージェントによる危険なコマンド実行を防ぐ
- 読み取り専用コマンドや安全なコマンドのみを許可
- 設定ファイルで柔軟にパターンを定義可能

## Phase 1: 初期実装

Phase 1では、以下の機能に集中します：

- ✅ AWSコマンドのサブコマンドレベルでの許可/拒否
- ✅ 設定ファイル（YAML）によるパターン定義
- ✅ 基本的なパターンマッチングロジック
- ✅ 実コマンドの実行とプロキシ機能

## アーキテクチャ

```
┌─────────────┐
│ Claude Code │
└──────┬──────┘
       │ safe-command -- aws s3 ls
       ▼
┌────────────────────────────┐
│    safe-command (Proxy)    │
│  ┌──────────────────────┐  │
│  │ 1. Parse Arguments   │  │
│  └──────────┬───────────┘  │
│             ▼              │
│  ┌──────────────────────┐  │
│  │ 2. Load Config       │  │
│  │    (safe-command.yaml)│  │
│  └──────────┬───────────┘  │
│             ▼              │
│  ┌──────────────────────┐  │
│  │ 3. Pattern Match     │  │
│  │    (aws.ts)          │  │
│  └──────────┬───────────┘  │
│             ▼              │
│        Allowed? ───No──▶ Error
│             │Yes           │
│             ▼              │
│  ┌──────────────────────┐  │
│  │ 4. Execute Command   │  │
│  └──────────┬───────────┘  │
└─────────────┼──────────────┘
              ▼
       ┌─────────────┐
       │  AWS CLI    │
       └─────────────┘
```

## 設定ファイル仕様

### 設計方針

safe-commandの設定ファイルは、以下の原則に基づいて設計されています：

1. **シンプル第一**: 文字列配列によるパターン定義で、学習コストを最小限に
2. **セキュアバイデフォルト**: 明示的に許可されたパターンのみ実行可能（デフォルト拒否）
3. **可読性重視**: YAMLコメントを活用して、パターンの意図を明確に
4. **拡張性の保持**: 将来的な機能追加に対応できる構造

### ファイル名

`safe-command.yaml` (プロジェクトルートまたは`~/.config/safe-command/safe-command.yaml`)

### フォーマット

```yaml
# safe-command 設定ファイル
# デフォルト: 全てのコマンドは拒否される
# patterns に明示的に記載されたパターンのみ許可される

commands:
  aws:
    patterns:
      # ============================================
      # Read-only operations (推奨: AIエージェント向け)
      # ============================================

      # 汎用的な読み取りコマンド
      - "* list-*"      # 任意のサービスの list- コマンド
      - "* get-*"       # 任意のサービスの get- コマンド
      - "* describe-*"  # 任意のサービスの describe- コマンド

      # S3 読み取り操作
      - "s3 ls*"        # バケット/オブジェクト一覧

      # Identity 確認
      - "sts get-caller-identity"  # 現在のIAMユーザー/ロール確認

      # ============================================
      # 注意: 以下は書き込み操作のため、通常はコメントアウト推奨
      # ============================================
      # - "s3 cp*"      # S3へのファイルコピー
      # - "s3 rm*"      # S3からのファイル削除
      # - "ec2 run-instances*"  # EC2インスタンス起動
```

### スキーマ定義

```typescript
type SafeCommandConfig = {
  commands: {
    [commandName: string]: {
      patterns: string[];
    };
  };
};
```

## CLIインターフェース

### 基本的な使用方法

```bash
# 使用方法
safe-command [options] -- <command> [args...]

# 例
safe-command -- aws s3 ls
safe-command -- aws ec2 describe-instances
safe-command -- aws sts get-caller-identity

# safe-command自体のオプションも使用可能（将来的に）
# safe-command --dry-run -- aws s3 rm s3://bucket/file.txt
```

### 実行結果

#### 許可された場合

実コマンドの標準出力/標準エラーをそのまま返します。終了コードも実コマンドの終了コードをそのまま返します。

```bash
$ safe-command -- aws s3 ls
2024-01-01 12:00:00 my-bucket-1
2024-01-01 12:00:00 my-bucket-2
```

#### 拒否された場合

エラーメッセージを標準エラーに出力し、終了コード1で終了します。

```bash
$ safe-command -- aws s3 rm s3://my-bucket/file.txt
Error: Command not allowed: aws s3 rm s3://my-bucket/file.txt
No matching pattern found in safe-command.yaml
```

## パターンマッチング仕様

### マッチングルール

1. **完全一致**: パターンに`*`が含まれない場合、引数が完全に一致する必要があります
2. **ワイルドカード**: `*`は0文字以上の任意の文字列にマッチします
3. **OR条件**: 複数のパターンのいずれかにマッチすれば許可されます
4. **大文字小文字**: 区別されます（AWSコマンドはケースセンシティブなため）

### マッチング例

設定:
```yaml
commands:
  aws:
    patterns:
      - "s3 ls*"
      - "ec2 describe-instances"
```

| コマンド | マッチ | 理由 |
|---------|--------|------|
| `aws s3 ls` | ✅ | `s3 ls*`にマッチ |
| `aws s3 ls s3://bucket` | ✅ | `s3 ls*`にマッチ |
| `aws s3 cp` | ❌ | どのパターンにもマッチしない |
| `aws ec2 describe-instances` | ✅ | 完全一致 |
| `aws ec2 describe-instances --region us-east-1` | ❌ | 完全一致せず |

### パターン設計のベストプラクティス

```yaml
commands:
  aws:
    patterns:
      # 👍 推奨: 読み取り専用コマンドを許可
      - "* list-*"
      - "* get-*"
      - "* describe-*"
      - "sts get-caller-identity"

      # ⚠️ 注意が必要: 書き込みコマンドは慎重に
      # - "s3 cp*"
      # - "ec2 run-instances*"

      # 🚫 非推奨: 広範すぎるパターン
      # - "*"  # 全てのコマンドを許可してしまう
```

## プロジェクト構造

```
safe-command/
├── src/
│   ├── index.ts        # エントリーポイント、CLI引数解析
│   ├── config.ts       # 設定ファイル読み込み
│   ├── matcher.ts      # 汎用パターンマッチングロジック
│   ├── executor.ts     # コマンド実行
│   └── aws.ts          # AWS固有のロジック・パターン定義
├── examples/
│   └── safe-command.yaml  # 設定ファイルの例
├── tests/
│   ├── matcher.test.ts
│   ├── aws.test.ts
│   └── integration.test.ts
├── package.json
├── tsconfig.json
├── SPEC.md            # この仕様書
└── README.md          # ユーザー向けドキュメント
```

### 各ファイルの役割

#### `src/index.ts`

- CLIのエントリーポイント
- コマンドライン引数の解析
- 全体のフロー制御

```typescript
// 擬似コード
const args = process.argv.slice(2);

// "--" を探してオプションとコマンドを分離
const delimiterIndex = args.indexOf('--');
if (delimiterIndex === -1) {
  console.error('Error: Missing "--" delimiter');
  process.exit(1);
}

const options = args.slice(0, delimiterIndex);
const [command, ...commandArgs] = args.slice(delimiterIndex + 1);

const config = await loadConfig();
const allowed = await checkAllowed(command, commandArgs, config);

if (allowed) {
  await executeCommand(command, commandArgs);
} else {
  console.error(`Error: Command not allowed: ${command} ${commandArgs.join(' ')}`);
  process.exit(1);
}
```

#### `src/config.ts`

- 設定ファイルの読み込み
- 設定ファイルのバリデーション
- デフォルト設定の処理

#### `src/matcher.ts`

- 汎用的なパターンマッチングロジック
- ワイルドカードの処理
- 他のコマンド（将来的にkubectl等）でも再利用可能

```typescript
export function matchPattern(pattern: string, target: string): boolean {
  // ワイルドカードを正規表現に変換してマッチング
}
```

#### `src/executor.ts`

- 実コマンドの実行
- 標準出力/標準エラーの処理
- 終了コードの処理

```typescript
export async function executeCommand(
  command: string,
  args: string[]
): Promise<void> {
  // Bun.spawnを使用してコマンド実行
}
```

#### `src/aws.ts`

- AWSコマンド固有のロジック
- AWS CLIのサブコマンド構造の解析
- 将来的にAWS固有の前処理・後処理を追加する際の拡張ポイント

```typescript
export function parseAwsCommand(args: string[]): string {
  // aws s3 ls s3://bucket → "s3 ls s3://bucket"
  return args.join(' ');
}
```

## 技術スタック

### ランタイム

- **Bun**: 高速なJavaScript/TypeScriptランタイム
  - 理由: 高速な起動、TypeScriptネイティブサポート、優れた開発体験

### 言語

- **TypeScript**: 型安全性とコード品質の向上

### 依存関係

- **js-yaml** または Bun標準ライブラリ: YAML設定ファイルの読み込み
- **Bun.spawn**: コマンド実行

### 開発ツール

- **Bun test**: ユニットテスト
- **TypeScript**: 型チェック

## 実装フロー

### 1. プロジェクトセットアップ

```bash
bun init
```

### 2. 設定ファイル読み込み

1. `./safe-command.yaml`を探す
2. なければ`~/.config/safe-command/safe-command.yaml`を探す
3. どちらもなければエラー

### 3. パターンマッチング

```typescript
function isAllowed(command: string, args: string[], config: SafeCommandConfig): boolean {
  const commandConfig = config.commands[command];
  if (!commandConfig) return false;

  const argsString = args.join(' ');

  for (const pattern of commandConfig.patterns) {
    if (matchPattern(pattern, argsString)) {
      return true;
    }
  }

  return false;
}
```

### 4. コマンド実行

```typescript
async function executeCommand(command: string, args: string[]): Promise<void> {
  const proc = Bun.spawn([command, ...args], {
    stdout: 'inherit',
    stderr: 'inherit',
  });

  const exitCode = await proc.exited;
  process.exit(exitCode);
}
```

## エラーハンドリング

### 設定ファイルが存在しない

```
Error: Configuration file not found
Searched locations:
  - ./safe-command.yaml
  - ~/.config/safe-command/safe-command.yaml

Please create a configuration file. See examples/ for sample configuration.
```

### パターンにマッチしない

```
Error: Command not allowed: aws s3 rm s3://my-bucket/file.txt
No matching pattern found in safe-command.yaml

To allow this command, add a pattern like:
  commands:
    aws:
      patterns:
        - "s3 rm*"
```

### 実コマンドが存在しない

実コマンドの実行エラーをそのまま返します。

```
Error: command not found: aws
```

### YAML構文エラー

```
Error: Failed to parse safe-command.yaml
Syntax error at line 5: unexpected token

Please check your YAML syntax.
```

## セキュリティ考慮事項

### 安全なデフォルト

- デフォルトでは**何も許可しない**
- 設定ファイルで明示的に許可されたコマンドのみ実行可能

### パターン設計のガイドライン

1. **最小権限の原則**: 必要最小限のコマンドのみ許可
2. **読み取り専用を優先**: `list-`, `get-`, `describe-`などの読み取りコマンドを優先
3. **ワイルドカードの注意**: `*`単体は避け、具体的なプレフィックスを指定

### 環境変数

- `AWS_ACCESS_KEY_ID`等の環境変数はそのまま実コマンドに渡される
- 機密情報のフィルタリングはPhase 2で検討

## 将来の拡張性（Phase 2以降）

Phase 1実装後に検討する機能：

### 他のコマンド対応

```typescript
// src/kubectl.ts
// src/terraform.ts
```

```yaml
commands:
  kubectl:
    patterns:
      - "get *"
      - "describe *"
  terraform:
    patterns:
      - "plan*"
      - "show*"
```

### ログ・監査機能

```yaml
logging:
  enabled: true
  file: ~/.config/safe-command/audit.log
```

### Dry-run機能

```bash
safe-command --dry-run -- aws s3 rm s3://bucket/file.txt
# Would execute: aws s3 rm s3://bucket/file.txt
# Result: DENIED (no matching pattern)
```

### 環境変数フィルタリング

```yaml
commands:
  aws:
    patterns:
      - "s3 ls*"
    env_allow:
      - AWS_PROFILE
      - AWS_REGION
    env_deny:
      - AWS_SECRET_ACCESS_KEY  # 機密情報を隠す
```

### インタラクティブモード

```bash
safe-command -- aws s3 rm s3://bucket/file.txt
# Command not allowed: aws s3 rm s3://bucket/file.txt
# Do you want to add this pattern to safe-command.yaml? [y/N]
```

## まとめ

safe-commandは、AIエージェントが実行できるコマンドを安全に制限するシンプルなプロキシツールです。Phase 1では、AWS CLIコマンドの基本的な許可/拒否機能を実装し、設定ファイルベースで柔軟にパターンを定義できるようにします。

将来的には、他のコマンドへの対応やログ機能など、より高度な機能を追加していく予定です。
