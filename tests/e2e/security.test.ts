...
import { setupCustomFixture, setupFixture } from "./helpers/fixtures";
...
describe("E2E: Security - Malicious Attack Patterns", () => {
	...
	describe("Command Injection Attacks", () => {
		...
		test("should safely handle backticks in arguments", async () => {
			...
			try {
				const result = await runSafeCommand(["echo", "safe`whoami`"], tempDir);
...
`;
			...
			try {
				...
				const result1 = await runSafeCommand(["echo", "test[123]"], tempDir);
				expect(result1.exitCode).toBe(0);
...
`;
			...
			try {
				const result = await runSafeCommand(["echo", "price$100"], tempDir);
				expect(result.exitCode).toBe(0);
			} finally {
				cleanup();
			}
...
`;
			...
			try {
				const result = await runSafeCommand(["echo", "func(args)"], tempDir);
				expect(result.exitCode).toBe(0);
			} finally {
				cleanup();
			}
...
`;
		...
		test("should handle Unicode characters", async () => {
			...
			try {
				const result = await runSafeCommand(["echo", "safe🚀日本語"], tempDir);
				expect(result.exitCode).toBe(0);
			} finally {
				cleanup();
			}
...
`;
			...
			try {
				...
				const result2 = await runSafeCommand(["echo", "aXXXbYYYc"], tempDir);
	...
	describe("Configuration Bypass Attempts", () => {
		...
		test("should not allow command name injection", async () => {
			...
			try {
				...
				const result = await runSafeCommand(["echo;rm", "-rf", "/"], tempDir);
				expect(result.exitCode).toBe(1);
				expect(result.stderr).toContain("not configured");
			} finally {
				cleanup();
